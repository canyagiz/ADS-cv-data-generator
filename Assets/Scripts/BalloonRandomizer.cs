...
public class BalloonRandomizer : Randomizer
{
    ...
    [Header("Color Settings (YENİ)")]
    [Tooltip("Parent objenin (Uçak gövdesi) alabileceği renkler.")]
    public Gradient parentColors;

    [Tooltip("Child objenin (WhiteBaloon) alabileceği renkler.")]
    public Gradient childColors;

    [Tooltip("Child objenin tam ismi (Hiyerarşideki ismiyle aynı olmalı).")]
    ...
    bool SpawnBalloon(GameObject prefab)
    {
        ...
        if (positionFound)
        {
            ...
            // ---------------------------------------------------------
            // RENK RANDOMİZASYONU (YENİ EKLENEN KISIM)
            // ---------------------------------------------------------

            // 1. Parent Rengi (Ally-plane)
            Renderer parentRenderer = balloon.GetComponent<Renderer>();
            if (parentRenderer != null)
            {
                // Gradient üzerinden rastgele bir zaman (0.0 ile 1.0 arası) seçip rengi alıyoruz
                parentRenderer.material.color = parentColors.Evaluate(Random.value);
            }

            // 2. Child Rengi (WhiteBaloon)
            ...
            if (childTransform != null)
            {
                ...
                {
                    // Child için farklı bir renk seçiyoruz
